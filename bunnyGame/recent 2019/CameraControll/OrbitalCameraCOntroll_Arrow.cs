...
public class OrbitalCameraCOntroll_Arrow : MonoBehaviour
{
    LayerMask layerMask ;
    ...
    void Update()
    {
        ...
        if (GUN.PlayerMaster.Instance.KeyboardMouse)
        {
            if (GUN.PlayerMaster.Instance.AxisControlls.ContainsKey(KEYS.ControllsKeyboardMouse.Instance.Camera))
            {
                ...
                input += new Vector2(/*(Input.GetAxis(Xvalue))*/0 * sensitivity, /*-Input.GetAxis(Yvalue)*/0 * sensitivity);//Controller
            }
            else
            {
